...
class handDetector():
    ...
    def fingersUp(self):
        ...
        if self.lmList[tipIds[0]][1] < self.lmList[tipIds[0] -1][1]:  #b
            fingers.append(1)
        else:
            fingers.append(0) # c
        for id in range(1,5): # index for the fingers from index finger to pinky finger 
            if self.lmList[tipIds[id]][2] < self.lmList[tipIds[id] -2] [2]: #a
                fingers.append(1)                                            
            else:                 # b
                fingers.append(0) #
        return fingers
