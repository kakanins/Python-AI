...
class FaceMesh():
    ...
    def detectFacialLandmarks(self, image, face_mesh):
        ...
        output_image = image[:,:,::-1].copy() #to copy the image from the provided parameter
    ...
    def isOpen(self, image, results, face_part, threshold=5):
    ...
    def getSize(self, image, face_landmarks, INDEXES):
        image_height, image_width, _ = image.shape
        INDEXES_LIST = list(itertools.chain(*INDEXES)) #a
        landmarks = []
        for INDEX in INDEXES_LIST:                            #
            landmarks.append([int(face_landmarks.landmark[INDEX].x *image_width), int(face_landmarks.landmark[INDEX].y * image_height)]) #b
        _, _, width, height = cv2.boundingRect(np.array(landmarks)) #c
    ...
    def masking(self, image, filter_img, face_landmarks, face_part, INDEXES):
        ...
        try:
            ...
            _, face_part_height, landmarks = self.getSizeimage(face_landmarks, INDEXES)
            ...
            return annotated_image # return masked image

cap = cv2.VideoCapture(0) #a
mp_face_mesh = mp.solutions.face_mesh #b
